...
H.delta = 1/sqrt(2);
Hi.num = est.H.wc^2;
Hi.den = [1 2*est.H.delta*est.H.wc est.H.wc^2];
...
t_delta1 = 5;    % Period of the step in experiment 1
M = 8;          % Number of step of the steircase function reference (To use on simulink) (maybe not necessary)

for n = 1:M
    steady.I(1+(n-1)*(t_delta1-2*t_tran1)/t:n*(t_delta1-2*t_tran1)/t,1) = I.signals.values((t_tran1+(n-1)*t_delta1)/t:(t_delta1-t_tran1+(n-1)*t_delta1)/t,1);    % Discard  transient, leaving only steady state samples
    steady.V(1+(n-1)*(t_delta1-2*t_tran1)/t:n*(t_delta1-2*t_tran1)/t,1) = V.signals.values((t_tran1+(n-1)*t_delta1)/t:(t_delta1-t_tran1+(n-1)*t_delta1)/t,1);
    steady.wm(1+(n-1)*(t_delta1-2*t_tran1)/t:n*(t_delta1-2*t_tran1)/t,1) = wm.signals.values((t_tran1+(n-1)*t_delta1)/t:(t_delta1-t_tran1+(n-1)*t_delta1)/t,1);
end
...
dyn.I(1+(n-1)*(t_delta2-2*t_tran2)/t:n*(t_delta2-2*t_tran2)/t,1) = I.signals.values((t_tran2+(n-1)*t_delta2)/t:(t_delta2-t_tran2+(n-1)*t_delta2)/t,1);     % Discard  transient (dyn = dynamic)
dyn.V(1+(n-1)*(t_delta2-2*t_tran2)/t:n*(t_delta2-2*t_tran2)/t,1) = V.signals.values((t_tran2+(n-1)*t_delta2)/t:(t_delta2-t_tran2+(n-1)*t_delta2)/t,1);
dyn.wm(1+(n-1)*(t_delta2-2*t_tran2)/t:n*(t_delta2-2*t_tran2)/t,1) = wm.signals.values((t_tran2+(n-1)*t_delta2)/t:(t_delta2-t_tran2+(n-1)*t_delta2)/t,1);
dyn.acc(1+(n-1)*(t_delta2-2*t_tran2)/t:n*(t_delta2-2*t_tran2)/t,1) = acc.signals.values((t_tran2+(n-1)*t_delta2)/t:(t_delta2-t_tran2+(n-1)*t_delta2)/t,1);
