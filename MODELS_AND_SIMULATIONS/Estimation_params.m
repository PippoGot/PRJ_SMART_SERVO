...
ex1.t_tran = 1;             % Time of transient samples to delate in experiment 1
ex1.t_delta = 10;           % Period of the step in experiment 1
ex1.M = 20;                 % Number of step of the steircase function reference

ex1.thetam = reshape(data.signals(1).values,[],1);              % [deg]
ex1.I = reshape(data.signals(4).values,[],1);                   % [A]
ex1.V = reshape(data.signals(5).values,[],1);                   % [V]
ex1.wl = data.signals(2).values(:,2) * conv.rpm__to__rad_s;     % from [rpm] to [rad/s]
ex1.wm = ex1.wl / motor.gearbox;                                % [rad/s]

ex1.time = linspace(0, size(ex1.thetam,1)*t, size(ex1.thetam,1)); % [s]
...
ex2.thetam = reshape(data.signals(1).values,[],1);                  % [deg]
ex2.V = reshape(data.signals(5).values,[],1);                       % [V]
ex2.I = reshape(data.signals(4).values,[],1);                       % [A]
ex2.wl = data.signals(2).values(:,2) * conv.rpm__to__rad_s;         % from [rpm] to [rad/s]
ex2.wm = ex2.wl / motor.gearbox;                                    % [rad/s]

ex2.time = linspace(0, size(ex2.thetam,1)*t, size(ex2.thetam,1));   % [s]
...
for n = 1:ex2.M
    ...
    dyn.time(i1(n):i2(n),1) = ex2.time(1,i3(n):i4(n));
end
